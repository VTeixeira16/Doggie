...
//    private TiledMapManager tiledMapMan;
	...
	public PlayScreen(GameClass game) {
        ...
        // Carrega o mapa e configura --- DESATIVADO, ATUALMENTE ESTÁ SENDO EXECUTADO ATRAVÉS DA FUNÇÃO createTiles();
...
//        tiledMapMan.createTiles();
        ...
        //Cria o mundo Box2D, onde o primeiro valor é a gravidade horizontal e o segundo vertical.
        ...
        // Cria HUD, onde valores e textos serão exibidos na tela. Na prática é uma câmera com mesmos valores da gamecam.
        hud = new Hud(doggie);
...
//        coleiras.update(dt);
	...
	public void render(float delta) {
		...
		gamecam.position.set(
				doggie.body.getPosition().x * PPM,
				doggie.body.getPosition().y * PPM,
				0);
		gamecam.update();
...
//        coleiras.render(game.sb);
        ...
        if(debug)
        {
    		...
    		b2dCamera.position.set(
					doggie.body.getPosition().x,
					doggie.body.getPosition().y,
					0);
			b2dCamera.update();
		...
		// Como o sb está sendo executado no game, verificar possibilidade de exclusão dessas funções.
//        game.sb.begin();      
//        game.sb.end();
